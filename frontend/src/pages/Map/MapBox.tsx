...
const MapBox = ({ center, zoom, style, size, onMapTypeChange, mapType: initialMapType }: MapBoxProps) => {
            ...
            map.on("click", "unclustered-point", (e) => {
              ...
              if (popupRef.current && popupRef.current._lngLat) {
                ...
                if (
                  ...
                  navigate(`/exhibit?id=${properties.id}&mapType=${mapType}`);
                  return;
                }
              }
  ...
  }, [center, zoom, style, overlayData, mapType, isMobile, closePopup, showPopup, artworksLoading, navigate]);
            ...
            <div className="map-missing-panel absolute bottom-16 left-3 sm:bottom-[72px] sm:left-4 z-20 w-[280px] sm:w-[320px]">
              ...
              <ul className="map-missing-panel-list">
                {missingLocationData.map((piece) => (
                  <li key={piece.id}>
                    <Link to={`/exhibit?id=${piece.id}&mapType=${mapType}`}>

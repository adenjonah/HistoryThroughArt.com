...
function Catalog({ search, setArtPiecesArray, layout, sort, unitFilters }) {
  ...
  useEffect(() => {
    // O(1) access of the index of the art piece in the korusOrder
    const korusMap = new Map(korusOrder.map((id, index) => [id, index]));
      ...
      for (const artPiece of artPiecesData) {
        ...
        for (const imageName of artPiece.image) {
          ...
          try {
            ...
            logger.error(`Failed to fetch image ${imageName}:`, error);
          }
        }
      }
